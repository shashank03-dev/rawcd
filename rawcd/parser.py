...
class DiscParser:
    ...
    def parse(self, root: Path) -> DiscInspection:
        disc_root = Path(root)
        if not disc_root.exists():
            return DiscInspection(
                disc_type=DiscType.UNKNOWN,
                label="Unknown disc",
                warnings=[f"Path not found: {disc_root}"],
            )

        if self._has_protected_layout_marker(disc_root):
            return protected_media_inspection()
    ...
    def parse_tool_output(self, root: Path, stderr: str) -> DiscInspection:
        if is_protected_media_error(stderr):
            return protected_media_inspection()
        return self.parse(root)

    def _has_protected_layout_marker(self, root: Path) -> bool:
        video_ts = self._case_insensitive_child(root, "VIDEO_TS")
        if video_ts is None or not video_ts.is_dir():
            return False
        found_names = {path.name.upper() for path in video_ts.iterdir()}
        return any(marker in found_names for marker in self.PROTECTED_LAYOUT_MARKERS)
...
def protected_media_inspection() -> DiscInspection:
    return DiscInspection(
        disc_type=DiscType.PROTECTED_MEDIA,
